...
use gltf::{Document, Gltf, image};
use nalgebra::Quaternion;
use pi_atom::Atom;
use pi_curves::curve::frame_curve::{frames::interplate_frame_values_step, FrameCurve};

use pi_engine_shell::prelude::*;
use pi_node_materials::prelude::BlockEmissiveTexture;
use pi_render::rhi::{VertexFormat, IndexFormat};
use pi_scene_context::prelude::*;
use pi_gltf as gltf;
use pi_scene_math::{Quaternion as MQuaternion, Vector3};

use crate::{interface::{FrameCurveType, GLTFCommands, GLTFAPI}, extras::particle::{Particle, MeshParticleMeshID}};
...
pub fn sys_gltf_decode(
    ...
    loadeds.drain().drain(..).for_each(|OpsGLTFLoaded(scene_id, loader, buffer_data)| {


        let mut factory = GLTFAPI { scene_id, commands: &mut cmd  };
        ...
        let root = commands.spawn_empty().id(); factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(root, scene_id));

        // factory.gltf_transform(root, None, None, None, None, None, );
        for node in gltf.nodes() {
            let node_entity = commands.spawn_empty().id(); factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(node_entity, root));
            factory.commands.transformcmds.create.push(OpsTransformNode::ops(scene_id, node_entity, String::from("name")));
            ...
            factory.gltf_transform(node_entity, t_translation, t_scaling, t_rotation, t_rotation_quternion, t_matrix);
            ...
            if let Some(mesh) = node.mesh() {
                ...
                for primitive in mesh.primitives() {
                    ...
                    factory.commands.meshcmds.create.push(OpsMeshCreation::ops(scene_id, mesh_entity, String::from("name")));
                    ...
                    primitive.attributes().for_each(|v| match v.0 {
                        ...
                        gltf::Semantic::Joints(set) => {
                                ...
                                .map(|v| v.into_u16().collect::<Vec<[u16; 4]>>())
                            {
                                ...
                                let (vertex_data_kind, format) = match set {
                                    0 => (EVertexDataKind::MatricesIndices, VertexFormat::Uint16x4),
                                    1 => (EVertexDataKind::MatricesIndices1, VertexFormat::Uint16x4),
                                    2 => (EVertexDataKind::MatricesIndices2, VertexFormat::Uint16x4),
                                    3 => (EVertexDataKind::MatricesIndices3, VertexFormat::Uint16x4),
                        ...
                        gltf::Semantic::Weights(set) => {
                                ...
                                .map(|v| v.into_f32().collect::<Vec<[f32; 4]>>())
                            {
                                ...
                                let (vertex_data_kind, format) = match set {
                                    0 => (EVertexDataKind::MatricesWeights, VertexFormat::Float32x4),
                                    1 => (EVertexDataKind::MatricesWeights1, VertexFormat::Float32),
                                    2 => (EVertexDataKind::MatricesWeights2, VertexFormat::Float32x2),
                                    3 => (EVertexDataKind::MatricesWeights3, VertexFormat::Float32x3),
                                    _ => panic!("uv not surpport overflow 4"),
                                };
                        ...
                        .and_then(|i| materials.get(i).cloned())
                    {
                        if let Some(extras) = material.extras(){
                            ...
                            factory.gltf_extras_material(mesh_entity, idmat, extras, &textures, &root_path);
                        ...
                        factory.gltf_default_material(mesh_entity.clone(), commands.spawn_empty().id());
                    }
                    ...
                    if let Some(extras) = node.extras(){
                        if let Some(mesh_particle_cfg) = extras.get("meshParticle"){
                            ...
                            factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(mesh_entity, scene_id));
                        } else {
                            factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(mesh_entity, node_entity));
                        }
                    } else {
                        factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(mesh_entity, node_entity));
                    }
                    ...
                    factory.gltf_geometry(mesh_entity.clone(), id_geo, vertex_buffer_desc, indices_desc);
            ...
            node.children().for_each(|child| {
                if let Some(entity) = node_map.get(&node.index()) {
                    if let Some(child) = node_map.get(&child.index()) {
                        ...
                        factory.commands.transformcmds.tree.push(OpsTransformNodeParent::ops(*child, *entity));
                    }
                }
            });
        }
                                    ...
                                    match channel.sampler().interpolation() {
                                        gltf::animation::Interpolation::Linear => {
                                            let asset_curve = if let Some(curve) = factory
                                                .gltf_check_anim_curve(&key_curve)
                                            {
                                                curve
                                            } else {
                                        ...
                                        gltf::animation::Interpolation::Linear => {
                                            let asset_curve = if let Some(curve) = factory
                                                    ...
                                                    inputs.zip(rotations.into_f32())
                                                {
                                                    ...
                                                    let euler_angles =
                                                        MQuaternion::from_quaternion(Quaternion::new(
                                                            rotation[0],
                                                            rotation[1],
                                                            rotation[2],
                                                            rotation[3],
